...
function calculator( button ){
    if( button.type == "operator" ){
      ...
      // FORMAT THE RESULT
        ...
        let text = document.createTextNode(op);
        li_node.appendChild(text);
        log.appendChild(li_node);
...
function showLogdata() {
    //local storage stores data in format [{operand,op,operange,=,result}]
    var  historyData = JSON.parse(localStorage.getItem("history")) || [];

    for (var i = 0;i<historyData.length; i++) {
        let node = document.createElement("LI");
        let expr = historyData[i];
        let textNode = document.createTextNode(expr);
